...
export function writeRecord(uploadDir: string, matchJobId: string, record: VerificationRecord): void {
  try {
    ...
    console.error(
      `[RetryVerifyStore] Failed to write record for ${matchJobId} range ${record.segmentIndex}: ${e?.message || e}`,
    );
  }
}
...
export function readRecord(
  ...
  try {
    ...
    console.warn(`[RetryVerifyStore] Corrupt record ${fileName(matchJobId, segmentIndex)}: ${e?.message || e}`);
    return null;
  }
}
