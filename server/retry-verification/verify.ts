/**
 * The RETRY candidate/verification system — a standalone, exact copy of
 * server/verification/ used ONLY by the manual Retry endpoints (segment
 * Retry + gap Retry). The bulk matching pass still runs on the original
 * server/verification/ system; changes here never affect it, and vice versa.
 *
 * Records are written in the exact same format and to the exact same
 * `<matchJobId>_verify_<rangeIndex>.json` files, so the UI, candidates
 * endpoints, retry-status, and verdict overrides all keep working unchanged.
 * ===========================================================================
...
export async function verifyMatchedSegments(req: VerifyRequest): Promise<VerifyResult> {
  ...
  if (total === 0) {
    console.log('[RetryVerify] No matched segments — nothing to verify.');
  ...
  if (!req.shortVideoPath || !req.movieVideoPath) {
    ...
    console.warn(`[RetryVerify] ${reason}.`);
  ...
  console.log(
    `[RetryVerify] Verifying ${total} matched range(s) with Gemini (model=${status.model}, ` +
  ...
  console.log(
    `[RetryVerify] Done: ${summary.accepted} accepted, ${summary.rejected} rejected, ` +
...
async function verifyOneRange(
  ...
  if (!shortClip) {
    ...
    console.warn(`[RetryVerify] Range ${segmentIndex} (${fmt(primary.shortStart)}-${fmt(primary.shortEnd)}): ${reason}.`);
  ...
  try {
    for (let i = 0; i < candidates.length; i++) {
      ...
      try {
        ...
        console.warn(
          `[RetryVerify] Range ${segmentIndex} candidate ${i}: Gemini call threw (${e?.message || e}) — unverifiable.`,
        );
      } finally {
      ...
      if (verdict.same && trusted) {
        ...
        console.log(
          `[RetryVerify] Range ${segmentIndex} (${fmt(primary.shortStart)}-${fmt(primary.shortEnd)}) ` +
...
function collectCandidates(
  ...
  if (rawAlternates.length !== alternates.length) {
    console.log(
      `[RetryVerify] Candidate filter for clip ${fmt(primary.shortStart)}s–${fmt(primary.shortEnd)}s: ` +
...
export async function recheckSegment(req: RecheckRequest): Promise<RecheckResult> {
  if (!req.shortVideoPath || !req.movieVideoPath || !geminiConfigured()) {
    ...
    console.warn(`[RetryVerify] Re-check of range ${req.segmentIndex} refused: ${message}.`);
    const record: VerificationRecord = {
  ...
  console.log(`[RetryVerify] Manual re-check requested for range ${req.segmentIndex}.`);
  req.onLog?.(
